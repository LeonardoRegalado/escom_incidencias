...
<h4 class="text-2xl font-semibold">Registrar una incidencia</h4>
<p class="leading-relaxed mt-1 mb-4 text-blueGray-500">
  Completa este formulario para registrar un reporte ciudadano.
</p>

<div class="relative w-full mb-3 mt-8">
  <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2">Título</label>
  <input v-model="titulo" type="text" class="input" placeholder="Ej. Fuga de agua" />
</div>

<div class="relative w-full mb-3">
  <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2">Descripción</label>
  <textarea v-model="descripcion" class="input" rows="3" placeholder="Describe lo que ocurre..." />
</div>

<div class="relative w-full mb-3">
  <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2">Ubicación</label>
  <input v-model="ubicacion" type="text" class="input" placeholder="Calle y referencia" />
</div>

<div class="relative w-full mb-3">
  <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2">Foto</label>
  <input type="file" @change="handleFile" class="input bg-white" accept="image/*" />
</div>

<div class="text-center mt-6">
  <button
    @click="registrarReporte"
    class="bg-blueGray-800 text-white text-sm font-bold uppercase px-6 py-3 rounded shadow hover:shadow-lg"
  >
    Enviar reporte
  </button>
</div>
...
<p v-if="mensaje" class="text-green-500 mt-3 text-sm text-center">{{ mensaje }}</p>
...
export default {
  ...
  methods: {
    ...
    async registrarReporte() {
      ...
      formData.append('id_usuario', this.usuarioLogeado?.id || ''); // o null si no está logeado
      formData.append('titulo', this.titulo);
      formData.append('descripcion', this.descripcion);
      formData.append('ubicacion', this.ubicacion);
      formData.append('latitud', this.latitud);
      formData.append('longitud', this.longitud);
      formData.append('foto', this.foto); // imagen seleccionada

        await axios.post("http://localhost:3000/reportes", formData, {
          headers: {
        ...
        this.titulo = this.descripcion = this.ubicacion = "";
        this.foto = null;
      } catch (err) {
        console.error(err);
        this.error = err.response?.data?.error || "Error al enviar el reporte.";
      }
    }
  }
};
